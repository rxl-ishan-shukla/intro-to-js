...
JavaScript (JS) is a lightweight, high-level, interpreted or JIT compiled scripting language conforming to the ECMAScript (ES) standard with first-class functions. It is known for its client-side execution, dynamic typing, single-threaded nature and support for multiple programming paradigms.

<details>
  <summary> <b>Breakdown of Technical Aspects</b> </summary>

- **Scripting Language:** Unlike compiled languages, JavaScript code is interpreted directly by the browser's JavaScript engine, eliminating the need for pre-compilation.

- **Client-Side vs Server-Side:** Traditionally, JavaScript executes on the client-side (user's browser) for dynamic web page behavior. However, modern frameworks like Node.js enable server-side scripting with JavaScript as well.

- **Single-Threaded:** JavaScript is single-threaded, meaning it can only execute one task at a time. This can be a limitation for complex applications, but techniques like event loops help manage asynchronous operations.

- **Dynamically Typed:** JavaScript is dynamically typed, meaning variables don't have a pre-defined data type. The type is associated with the data value at runtime, offering flexibility but potentially leading to runtime errors.

- **Multi-Paradigm:** JavaScript supports multiple programming paradigms like object-oriented, imperative, and functional programming, allowing developers to choose the approach that best suits the task.

- **ECMAScript (ES):** JavaScript adheres to the ECMAScript (ES) specification, a standardized version of the language that ensures consistency across different browsers. New features are added through yearly ES releases.

- **First-Class Function:** A language is said to have first-class functions when function in that language are treated like any other variable.

</details>

## What is ECMAScript?

ECMAScript (ES) is a standardization document languages should have, ensuringcompatibility across different web browsers. Think of it as a rulebook that JavaScript follows.

## Data Types

### Primitive Data Types 

These are fundamental data types that directly represent a single value. They include:
...
These are reference types that store collections of data or complex relationships.

- Object

## Falsy Values

In JavaScript, certain values are considered "falsy" when evaluated in a Boolean context. There values translate to false when used in conditions or other situations that require a true or false answer. Here's the breakdown:

- **false:** The boolean value itself.
- **0:** The number zero, including both positive and negative zero.
- **"":** An empty string, with no characters.
- **null:** Represents the initial absence of a value.
- **undefined:** Represents a variable that's declared but not assigned a value.
- **NaN:** Stands for "Not a Number", resulting from an operation that dosen't yield a valid number.

Essentially, these six values are treated as false-like in conditional statements, logical operations, and other contexts that rely on Boolean evaluation.

## Type Conversion

JavaScript supports 2 main mechanisms for type conversion:

1. **Implicit Type Conversion (Coercion):** This happens automatically behind the scenes when JavaScript attempts to use values of different data types in an operation. For example:
- Adding a number and a string: "10" + 2 becomes "12" (string + number becomes a string)
- Checking a non-empty string in a condition: if ("hello") { ... } evaluates to true (string is treated as truthy)

2. **Explicit Type Conversion:** You can explicitly convert a value from one type to another using functions like:
- *Number(value):* Converts a value to a number (if possible)
- *String(value):* Converts a value to a string
- *Boolean(value):* Converts a value to a boolean (truthy or falsy)

These functions provide more control over how you want to interpret the data and can be useful to avoid unexpected behavior from implicit conversions.

<details>
<summary><b>Numeric Conversion Rules</b></summary>

- undefined  -> NaN
- null       -> 0
- true       -> 1
- false      -> 0
- string     -> Whitespaces are tirmmed from the start and end of the string. If the remaining string is empty, the result is 0. Otherwise, the number string is converted to number. Else if there is an error it gives NaN.

</details>

<details>
<summary><b>Boolean Conversion Rules</b></summary>
...
In JavaScript, var, let, and const are all keywords used to declare variables, but they differ in three key aspects: scope, re-assignment, and hoisting.

<details>
  
<summary> Scope </summary>

- **var:** Has function-level or global scope. A variable declared with var is accessible throughout its entire enclosing function or globally if outside any function. This can lead to unintended consequences if variables with the same name are declared in different parts of your code.
- **let and const:** Have block-level scope. A variable declared with let or const is only accessible within the block (code wrapped in curly braces {}) where it's declared. This promotes better code organization and avoids naming conflicts.

</details>

<details>

<summary> Re-assignment </summary>

- **var:** Allows re-assignment of the value after declaration. You can change the value stored in a var variable throughout the code.
- **let:** Also allows re-assignment but within its block scope. You can change the value of a let variable within the block where it's declared.
- **const:** Does not allow re-assignment after declaration. The value assigned to a const variable must remain constant throughout the program.
...
- **var:** Undergoes hoisting, meaning variable declarations with var are treated as if they are declared at the beginning of their enclosing scope (function or global). This can lead to unexpected behavior if you access a var variable before its declaration in the code.
- **let and const:** let and const on the other hand are hoisted but not initialized. Referencing the variable in the block before the variable declaration results in a ReferenceError, because the variable is in a "temporal dead zone" from the start of the block until the declaration is processed.

</details>

### Best Practices
- In modern JavaScript, it's generally recommended to use const by default for variables that don't need to be re-assigned. This helps prevent accidental changes and improves code clarity.
- Use let when you need to re-assign the value within a block.
- Avoid var due to its potential for scoping issues and hoisting behavior.

## Comments

1. **Single-line:** These comments start with two forward slashes (//) and extend to the end of the line. They're useful for brief explanations or annotations next to specific lines of code.
2. **Multi-line:** These comments use the opening sequence **/*** followed by the content and then closed with ***/**. They allow you to document larger code blocks or provide more detailed explanations.

## Chaining Assignments

Chaining variables should be avoided because it creates implicit global variables in sloppy mode and in strict mode they are invalid references to non-existent variables.
...
Both function declarations and function expressions are ways to create functions in JavaScript, but they differ in some key aspects:
...
- **Hoisting:** Function declarations are hoisted. This means they are treated as if they are declared at the beginning of their scope (global or function scope) regardless of their actual position in the code. You can call a function declaration before its definition in the code.
- **Scope:** Function declarations have either global scope (if defined outside any function) or local scope (if defined within a function).
- **Example:** 
```JavaScript
function greet(name) {
   return "Hello, " + name + "!";
...
- **Syntax:** const functionName = function(parameter1, parameter2, ...) { ... }
- **Hoisting:** Function expressions are not hoisted. You cannot call a function expression before it's defined in the code.
- **Scope:** Function expressions have block scope. They are accessible only within the block where they are defined.
- **Example:** 
```JavaScript
...
The Arguments of a function are maintained in an array-like object. The arguments variable is "array-like", but not an array. It is array-like in that it has a numbered index and a length property. However, it does not possess all of the array-manipulation methods.

```JavaScript
function fun(a,b,c){
   console.log(arguments); // Arguments(3){0:1, 1:2, 2:3}
   return a + b + c;
}
...
```
## Arrow Function
Arrow functions, introduced in ES6 (ECMAScript 2015), provide a concise syntax for writing JavaScript functions. It does not have its own this, arguments, super, and are anonymous functions.
Example:
```JavaScript
const obj = {
   i: 10,
...
Higher-order functions (HOFs) are a powerful concept in JavaScript that deal with functions in a special way. Here's a breakdown of what they are and how they work:

**What are HOFs?**

- HOFs are functions that take one or more functions as arguments. These functions can be treated like values, passed around, and even returned from other functions.
- They allow you to write more concise, reusable, and expressive code by abstracting away common patterns and functionalities.

**Key characteristics**

- *Accept functions as arguments:* This is the core aspect of HOFs. They can take one or more functions as input, allowing you to provide custom logic for the HOF to execute.
- *Return a new function (optional):* Some HOFs also return a new function as their output. This can be useful for creating functions tailored to specific situations.

**Benefits of using HOFs**

- *Improved code reusability:* By encapsulating logic within functions and passing them around, you can avoid code duplication and promote modularity.
- *Cleaner and more concise code:* HOFs can help write complex logic in a more readable and maintainable way by separating concerns.
- *Declarative style:* HOFs often encourage a declarative programming style, where you focus on "what" needs to be done rather than the intricate "how."

**Common HOF examples:**

- map(): Applies a function to all elements in an array and returns a new array with the results.
...
JavaScript allows for the nesting of functions and grants the inner function full access to all the variables and functions defined inside the outer function. However, the outer function does not have access to the variables and functions defined inside the inner function. This provides a sort of encapsulation for the variables of the inner function.

## Destructuring

Destructuring assignment is a powerful feature in JavaScript that allows you to unpack values from arrays or objects into distinct variables in a concise and readable way. 

```JavaScript
const sections = ['general', 'reporters', 'patient', 'products'];
...
Both spread (...) and rest (...) operators look alike in JavaScript, but they serve distinct purposes:

### Spread Operator (...)

Expands an iterable object (array, string, object) into individual elements when used in various contexts.

Common use cases:
- Copying arrays (spread creates a shallow copy).
- Combining arrays (spreading multiple arrays into one).
- Passing arguments to functions (spreading function arguments).
- Creating object literals with concise property definitions (spreading existing object properties).
...
const allProduce = [...fruits, ...veggies];

// Spreading arguments in a function
function sum(x, y, z) {
  return x + y + z;
}
...
Gathers remaining elements from an iterable object (array) into a new array when used in function parameter lists.

Essentially collects the "rest" of the elements not assigned to specific parameters.
...
Arrays are a fundamental data structure in JavaScript used to store collections of items. They are ordered, meaning elements have a specific position within the array, and can hold items of various data types (numbers, strings, objects, etc.)

  - Using Array constructor
  ```JavaScript
  let myArray1 = new Array(); // Empty array
  let myArray2 = new Array(10); // Array with 10 empty slots
  let myArray3 = new Array("apple", "banana", "cherry"); // Array with initial elements
  ```
  - Using Array Literal
  ```JavaScript
  let myArray1 = []; // Empty array;
  let myArray2 = [1,2,3]; // Array with initial elements
  ```
## Objects

In JavaScript, objects are powerful data structures used to store collections of key-value pairs. These key-value pairs define properties (the keys) and their corresponding values. Objects can also contain methods, which are functions associated with the object.

Here's a breakdown of object properties and methods:

### Properties

- Properties are named attributes associated with an object. They can hold various data types like numbers, strings, arrays, other objects, or even functions.
- You can access properties using dot notation (.) or bracket notation ([]). Bracket notation is useful when the property name is a variable or contains special characters.
...
console.log(person.hobbies[0]); // Output: "reading" (accessing element within array property)
```

### Methods

- Methods are functions defined within an object that act on the object's data. They are essentially properties that hold function values.
- Methods are invoked using dot notation followed by parentheses, optionally passing arguments.
...
- Objects provide a flexible way to organize related data and functionality.
- You can add or remove properties from objects dynamically after creation.
- Methods allow objects to encapsulate data and behavior, promoting modularity and reusability.

### Common Object Methods

- *Object.keys(object):* Returns an array containing the names of all enumerable properties of an object.
- *Object.values(object):* Returns an array containing the values of all enumerable properties of an object.
- *Object.entries(object):* Returns an array of key-value pairs ([key, value]) for each enumerable property of an object.
- *hasOwnProperty(property):* Returns a boolean indicating whether the object has a property with the specified name (inherited properties not included).
...
The meaning of "this" in JavaScript depends on the context in which it's used.
- **Inside a function:** When "this" is used within a method of an object, it refers to the object itself. The method can access the object's properties and other methods using "this".
```JavaScript
const person = {
  name: "Alice",
  greet: function() {
...
person.greet(); // Output: "Hello, my name is Alice"
```
- **Alone or in Global Scope:** In the global scope (outside any function), or when used alone without a specific context, "this" refers to the global object. In web browsers, the global object is typically the window object.
```JavaScript
...
Getters and setters are accessor properties in JavaScript that provide a way to control how you access and modify the values of an object's properties. They offer more control and flexibility compared to traditional property assignments.

### Getters
- Defined using the get keyword followed by a function within an object literal.
- This function is invoked whenever you try to access the corresponding property using dot notation.
- The function typically returns the value of the property, but it can also perform additional logic before returning the value.

### Setters

- Defined using the set keyword followed by a function within an object literal.
- This function is invoked whenever you try to assign a new value to the corresponding property using dot notation.
- The function typically takes one argument, which represents the new value being assigned.
- The setter can perform validation, data processing, or other operations on the new value before storing it in the property.

### Example

```JavaScript
const person = {
  _firstName: "John", // Use a convention like _ for private properties

  get firstName() {
    return this._firstName.toUpperCase(); // Return first name in uppercase
  },
...
console.log(person.firstName); // Output: JOHN (getter is called)

person.firstName = "Alice"; // setter is called
console.log(person.firstName); // Output: ALICE (getter is called again)

person.firstName = "A"; // setter is called with validation error
```

## Constructor Functions and new Keyword

In JavaScript, constructor functions and the new keyword are fundamental concepts for creating objects and working with object-oriented programming. Here's a breakdown of their roles:

### Constructor Functions

- **Purpose:** A constructor function is a special type of function designed to create and initialize objects.
- **Structure:** It typically follows these conventions: Starts with a capital letter (common practice). Can have parameters to accept values that will be used to initialize the object's properties. Often includes logic to set up the object's properties using this keyword to refer to the new object being created. By convention, it doesn't have an explicit return statement, as the new keyword handles object creation and return.
- **Example**
```JavaScript
...
- **Purpose:** The new keyword is used in conjunction with a constructor function to: Create a new empty object. Call the constructor function with the provided arguments (if any). Set the object's internal prototype to the constructor function's prototype property (for inheritance). Return the newly created and initialized object.
- **Example**
```JavaScript
...
- Constructor functions provide a blueprint for creating objects with specific properties and behaviors.
- The new keyword is essential for invoking a constructor function and creating new objects.
- Properties and methods defined within a constructor function become properties and methods of each object created using that constructor.

In essence, constructor functions act as templates, and the new keyword brings those templates to life as objects.
